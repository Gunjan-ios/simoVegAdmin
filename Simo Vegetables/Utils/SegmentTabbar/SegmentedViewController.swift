...
class SegmentedViewController: ParentClass {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        yPos =  CGFloat(STATUS_BAR_HEIGHT) + ParentClass.sharedInstance.iPhone_X_Top_Padding
    ...
    func layoutHeaderViewAndTabBar() {
        ...
        if let scrollView = scrollViewWithSubViewController(viewController: selectedViewController) {
            ...
            if (realHeight < headerView.maximumOfHeight + self.scrollView.contentInset.top) {
                var originY = realHeight - headerView.maximumOfHeight - self.scrollView.contentInset.top
                originY = originY + CGFloat(STATUS_BAR_HEIGHT);
            ...
            print("new \(headerView.maximumOfHeight + self.scrollView.contentInset.top)")
        }

        let segmentControllY = headerView.frame.maxY
        if segmentControllY <= yPos{
            segmentedControl.frame = CGRect(origin: CGPoint(x: 0, y: yPos), size: CGSize(width: self.view.frame.width, height: segmentedControlHeight))
           scrollView.frame = CGRect (x: 0, y:  Int(self.yPos), width: SCREEN_WIDTH , height: SCREEN_HEIGHT - Int(self.yPos))
...
//        segmentedControl.frame = CGRect(origin: CGPoint(x: 0, y: segmentControllY), size: CGSize(width: self.view.frame.width, height: segmentedControlHeight))
    ...
    func setControllersForSegments(contents: [(title: SegmentedControl.SegmentedItem, controller: UIViewController)], scrollTo index: Int = 0) {
        clearObserver()
        segmentedViewController.forEach( { $0.removeFromParent() })
...
//    func changeControllersTabName(names:[String]) {
////        clearObserver()
...
//        scrollView.frame = CGRect (x: 0, y:  Int(self.yPos), width: SCREEN_WIDTH , height: SCREEN_HEIGHT - Int(self.yPos))
    ...
    fileprivate func addContainerView() {
        ...
        scrollView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|[container]|", options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: ["container": containerView]))
        scrollView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "|[container]|", options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: ["container": containerView]))
    ...
    fileprivate func configureSegmentedController() {
        ...
        segmentedControl.selectedFont = UIFont.boldSystemFont(ofSize: 15)
    ...
    func loadSubControllerIfNeeded(_ index: Int? = nil) {
        ...
        if 0 <= currentIndex && currentIndex < segmentedViewController.count {
            ...
            if viewController.parent == nil {
                ...
                backgroundView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "|[v]|", options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: ["v": viewController.view]))
                backgroundView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|[v]|", options: NSLayoutConstraint.FormatOptions(rawValue: 0), metrics: nil, views: ["v": viewController.view]))
...
//                print("inner \(originY)")
//
////                self.printOn(pMessage: "inner \(originY)")
//
//                if originY == -50 {
//                    let imgV = headerView.view(withId: "12345")
//                    imgV!.isHidden = true
//                } else {
//                    let imgV = headerView.view(withId: "12345")
//                    imgV!.isHidden = false
...
//                print("outer \(realHeight)")
//
////                self.printOn(pMessage: "outer \(realHeight)")
//            }
//
//            scrollView.scrollIndicatorInsets = UIEdgeInsets(top: segmentedControl.frame.maxY - self.scrollView.contentInset.top, left: 0, bottom: 49, right: 0)
//        } else {
//            headerView.frame = CGRect(origin: headerView.frame.origin, size: CGSize(width: headerView.frame.width, height: headerView.maximumOfHeight + self.scrollView.contentInset.top))
//            print("new \(headerView.maximumOfHeight + self.scrollView.contentInset.top)")
////            self.printOn(pMessage: "new \(headerView.maximumOfHeight + self.scrollView.contentInset.top)")
//        }
//
//        let segmentControllY = headerView.frame.maxY
//        segmentedControl.frame = CGRect(origin: CGPoint(x: 0, y: segmentControllY), size: CGSize(width: self.view.frame.width, height: segmentedControlHeight))
